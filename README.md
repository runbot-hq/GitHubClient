...
> **Note:** CI workflows are defined but not yet active — the Actions runners are not wired up yet.
> PRs will show a `pending` merge status with no check runs attached; this is expected and not a build failure.
