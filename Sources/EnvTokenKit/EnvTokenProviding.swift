...
public protocol EnvTokenProviding: Sendable {
    ...
    ///
    /// - Important: Every conforming type **must** declare this `nonisolated`.
    ///   `NullEnvTokenProvider` and `StubEnvTokenProvider` are the reference
    ///   implementations — both carry the keyword explicitly. The compiler will
    ///   NOT warn if you omit it on a non-isolated struct; the silent actor hop
    ///   only manifests at runtime. Treat this as a load-bearing contract,
