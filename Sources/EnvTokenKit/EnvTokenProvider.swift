...
public final class EnvTokenProvider: EnvTokenProviding, Sendable {
    ...
    ///
    /// ## ProcessInfo vs getenv() — intentional divergence from OAuthService
    ...
    /// Do NOT "fix" this to `getenv()` for consistency — the divergence is the design.
    ///
    ...
    ///   If `RunnerPoller` is ever made concurrent, add an
    ///   `OSAllocatedUnfairLock<Bool>`-guarded `inFlight` flag here — do NOT
    ///   re-introduce the `warmUp()` scaffold removed in #74.
